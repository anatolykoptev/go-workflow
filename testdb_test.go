...
// advisory-lock scheme (dblock_test.go): different databases never contend, so
// no lock is needed and there is no per-subtest connection churn.
//
...
// requireTestDB validates that dsn refers to a database whose name contains
// "_test". Returns a non-empty error string if the name looks like a
// production database.
