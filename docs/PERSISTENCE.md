...
```go
store, err := workflow.NewFileStore("/path/to/workflows")
```
...
```go
store, err := workflow.NewSQLiteStore("/path/to/workflows.db")
```
...
```go
store, err := workflow.NewPostgresStore("postgres://user:pass@localhost:5432/go_workflow?sslmode=disable")
```
...
└── delegates to StoreBackend
    ├── FileBackend      (store_file.go)
    ├── SQLiteBackend    (store_sqlite.go)
    └── PostgresBackend  (store_postgres.go)
```
...
A conformance test suite (`store_conformance_test.go`) runs 11 subtests against every backend:
...
PostgreSQL tests require a `GO_WORKFLOW_POSTGRES_DSN` environment variable and are skipped otherwise.
