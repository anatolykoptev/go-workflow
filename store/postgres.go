...
func NewPostgresStore(dsn string) (*workflow.WorkflowStore, error) {
...
func (p *PostgresBackend) Save(w *workflow.Workflow) error {
...
func (p *PostgresBackend) Load(id string) (*workflow.Workflow, bool) {
	var data []byte
...
func (p *PostgresBackend) List(state workflow.WorkflowState) []*workflow.Workflow {
	return p.queryWorkflows(state, "")
}

// ListByOwner returns workflows owned by the given owner.
func (p *PostgresBackend) ListByOwner(owner string) []*workflow.Workflow {
	return p.queryWorkflows("", owner)
}

// FindByIdempotencyKey returns the first non-terminal workflow with the given key, or nil.
func (p *PostgresBackend) FindByIdempotencyKey(key string) *workflow.Workflow {
	var data []byte
	err := p.db.Get(&data,
...
func (p *PostgresBackend) Modify(id string, fn func(w *workflow.Workflow)) error {
	tx, err := p.db.Beginx()
	if err != nil {
...
// CleanAll removes all workflows from the database. Used for testing.
func (p *PostgresBackend) CleanAll() {
	p.db.MustExec("DELETE FROM workflows")
}
...
func (p *PostgresBackend) queryWorkflows(state workflow.WorkflowState, owner string) []*workflow.Workflow {
